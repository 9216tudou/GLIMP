...
from models.Model_NoGNN import LncLocNoGNN
...
def train_no_gnn_model():
    # 1. Configuration
    conf = config()
    conf.data_path = 'data/human/nonTATA/promoter80-300/train_keep_replace_80_300.csv'
    conf.res_dir = 'finalResult/ablation_no_gnn' # Output directory for this specific ablation
    ...
    for fold_idx, (train_idx, dev_idx) in enumerate(kf.split(dataset)):
        ...
        # Using the Model_NoGNN
        model = LncLocNoGNN(k=conf.k, embed_dim=conf.embed_dim, hidden_dim=conf.hidden_dim, use_bert=True)
