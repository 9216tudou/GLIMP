...
from models.Model_NoAttention import LncLocNoAttention
...
def train_no_attention_model():
    # 1. Configuration
    conf = config()
    conf.data_path = 'data/human/nonTATA/promoter300/train_keep_replace.csv'
    conf.res_dir = 'finalResult/ablation_no_attention' # Output directory for this specific ablation
    ...
    print(f"--- [消融实验] 开始无 Attention (Transformer) 模块训练 ---")
    ...
    for fold_idx, (train_idx, dev_idx) in enumerate(kf.split(dataset)):
        ...
        # Using the Model_NoAttention
        model = LncLocNoAttention(k=conf.k, embed_dim=conf.embed_dim, hidden_dim=conf.hidden_dim, use_bert=True)
    ...
    # We use the LAST val_loader (fold 5) for demonstration
