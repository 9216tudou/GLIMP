...
from models.Model_NoDNABERT import LncLocNoDNABERT
...
# from generate_bert_features import generate_bert_features # Not needed for NoDNABERT training
...
def train_no_dnabert_model():
    # 1. Configuration
    conf = config()
    conf.data_path = 'data/human/nonTATA/promoter80-300/train_keep_replace_80_300.csv'
    conf.res_dir = 'finalResult/ablation_no_dnabert' # Output directory for this specific ablation
    ...
    # We DO NOT generate bert features here
    bert_path = None 
    ...
    for fold_idx, (train_idx, dev_idx) in enumerate(kf.split(dataset)):
        ...
        # Using the Model_NoDNABERT
        model = LncLocNoDNABERT(k=conf.k, embed_dim=conf.embed_dim, hidden_dim=conf.hidden_dim)
